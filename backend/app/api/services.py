...
def list_services(db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    services = db.query(Service).filter(Service.store_id == user.store_id).order_by(Service.id).all()
    return [
        ServiceBase(
            id=service.id,
            name=service.name,
...
def update_service(service_id: int, payload: ServiceUpdate, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    service = db.query(Service).filter(Service.id == service_id, Service.store_id == user.store_id).first()
    if not service:
